...
public:
	...
	constexpr T& operator()(unsigned int i) { if (i == 0) return x; else if (i == 1) return y; else if (i == 2) return z; return w; }
	constexpr const T& operator()(unsigned int i) const { if (i == 0) return x; else if (i == 1) return y; else if (i == 2) return z; return w; }
	constexpr T& operator[](unsigned int i) { if (i == 0) return x; else if (i == 1) return y; else if (i == 2) return z; return w; }
	constexpr const T& operator[](unsigned int i) const { if (i == 0) return x; else if (i == 1) return y; else if (i == 2) return z; return w; }
